...
# NOTE on PLAT-04 (tablet optimization):
# The Expo app.json has supportsTablet: true for iOS and standard Android package.
# React Native's flexbox layout is inherently responsive - tablets get more screen space
# and the UI scales naturally. For v1, this basic responsive behavior is sufficient.
# Dedicated tablet layouts (split views, larger touch targets) are Phase 6+ polish work.

must_haves:
  truths:
    - "Expo app starts on iOS simulator, Android emulator, and web"
    - "File-based routing works with Expo Router"
    - "App displays API connection status on login screen"
  artifacts:
...
Full stack verification (requires Plan 01-02 backend running):
- `curl http://localhost:3000/health` returns {"status":"ok",...}
- `curl http://localhost:3000/health/db` returns {"status":"ok","database":"connected",...}
- Login screen displays "API: ok" confirming mobile-to-backend connection.
  </verify>
...
6. Both /health AND /health/db endpoints return 200 OK (confirms full backend stack)
</verification>

<success_criteria>
- Expo app exists at apps/mobile/
- App starts on iOS, Android, and web from single codebase (PLAT-01, PLAT-02, PLAT-03)
- Tablet support enabled via supportsTablet: true (PLAT-04 - responsive layout, not custom tablet UI for v1)
- Expo Router file-based navigation works
- Auth flow skeleton exists (login screen)
- App flow skeleton exists (home screen)
- API client can communicate with backend (both /health and /health/db)
